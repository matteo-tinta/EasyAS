...
import { TokenCollection } from '../../src/infrastructure/persitence/database';

describe('Refresh Endpoint', () => {
  composeTest(
    'when refresh token is not expired, issues a new refresh and access token',
    async ({ appUrl, client }) => {
      ...
      const onDbSessionId = await client.db().collection<TokenCollection>('tokens').findOne({
        refreshToken: refreshTokenBody.refreshToken,
      });

      expect(onDbSessionId?.sessionId).to.be.equal('sessionID_1');
    },
  );
