...
describe("Login Endpoint", () => {
    ...
    composeTest.concurrent("login should return an access token", async ({ appUrl, seed }) => {
        await seed("users", {
    ...
    composeTest.concurrent("login should return 401 if password is wrong", async ({ appUrl, seed }) => {
        seed("users", {
            username: "exampleUser",
            password: "strongPassword123"
        })
